...
[Slide-deck YAML]
        ↓ (THIS SKILL: compose-pptx-2)
[.pptx file]
```

This is an **alternative implementation of `compose-pptx`**. The inputs, the YAML
schema, the matching philosophy and the output are identical. The one deliberate
difference is **how the .pptx is built**:

- `compose-pptx` builds with the **python-pptx object model** (`add_slide`,
  `add_chart`, `add_table`).
- `compose-pptx-2` builds by **editing the raw OOXML** — unpack the template ZIP,
  edit the slide XML directly (with the Edit tool + small plumbing scripts), then
  repack. This is the method from `anthropics/skills:skills/pptx`. **No
  python-pptx is used in the build.** (The only python-pptx use anywhere is the
  read-only `inspect_template.py`, a Step-2 *inventory* fallback — never the build.)

It also adds the improvement the python-pptx skill lacks: a **clone-to-grid
solver** (`clone_grid.py`) for the case where a YAML slide needs a layout the
template has no sample for (e.g. a 2×4 card grid when the template only ships a
3-card row). See [The grid solver](#the-grid-solver-clone_gridpy).
...
| `clone_grid.py …` | Replicate a styled shape into an R×C grid (**the improvement**) | 4 |
...
Identical to `compose-pptx`. Understand what sample slides the template offers.
...
This is the part that differs from `compose-pptx`. Work in a scratch directory.
...
**share one chart part** — writing data into one corrupts the other. (This is the
same shared-part hazard `compose-pptx` documents and dodges by rebuilding charts
fresh with python-pptx.)

`set_chart_data.py` resolves it without python-pptx: it **forks** the chart part
...
This is the augmentation `compose-pptx` lacks — for the "novel layout" case where a
YAML slide needs a grid the template ships no sample for.
